...
export async function POST(request: NextRequest) {
  try {
    ...
    const [{ data: org }, { data: settings }, { data: profile }] = await Promise.all([
      supabase.from("organizations").select("id, name").eq("id", body.organizationId).maybeSingle(),
      supabase
        .from("organization_settings")
        .select("stripe_customer_id")
        .eq("organization_id", body.organizationId)
        .maybeSingle(),
      supabase.from("profiles").select("email").eq("auth_user_id", user.id).maybeSingle(),
    ]);
    ...
    if (!customerId) {
      ...
      await supabase
        .from("organization_settings")
        .upsert(
          { organization_id: org.id, stripe_customer_id: customer.id, subscription_plan: "starter" },
        ...
        : [
            {
              ...
              price_data: {
                ...
                product_data: {
                  name: "Kiosk POS Pro",
                  description: "Full team and operations access billed annually.",
                },
                unit_amount: 2000,
              },
            },
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Failed to start checkout." },
      { status: 500 }
    );
  }
}
