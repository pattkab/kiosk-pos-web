-- Production POS transaction engine.
-- The checkout RPC below performs sale creation, payment capture, and inventory deduction atomically.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_sales_org_receipt_number
    ON sales(organization_id, receipt_number)
    WHERE receipt_number IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_register_sessions_one_active_cashier
    ON register_sessions(organization_id, cashier_id)
    WHERE closed_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_products_org_barcode_active
    ON products(organization_id, barcode)
    WHERE is_active = TRUE AND barcode IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_products_org_sku_active
    ON products(organization_id, sku)
    WHERE is_active = TRUE AND sku IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_register_sessions_active
    ON register_sessions(organization_id, cashier_id, opened_at DESC)
    WHERE closed_at IS NULL;

DROP TRIGGER IF EXISTS on_sale_item_insert ON sale_items;
DROP FUNCTION IF EXISTS manage_inventory_on_sale();
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public'
          ...
          AND policyname = 'Members can view cash registers'
    ) THEN
        CREATE POLICY "Members can view cash registers"
    ...
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public'
          AND tablename = 'cash_registers'
          AND policyname = 'Managers can manage cash registers'
    ) THEN
        CREATE POLICY "Managers can manage cash registers"
    ...
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public'
          AND tablename = 'cash_registers'
          AND policyname = 'Members can create default cash registers'
    ) THEN
        CREATE POLICY "Members can create default cash registers"
